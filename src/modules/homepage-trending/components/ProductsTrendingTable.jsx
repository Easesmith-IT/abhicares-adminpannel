...
export default function ProductsTrendingTable({
  rows,
  isLoading,
  rowLoadingMap,
  onToggle,
    ...
    <Table>
      <TableHeader>
        <TableRow className="bg-slate-50/90">
          <TableHead className="sticky top-0 z-10 bg-slate-50/95">Product</TableHead>
        ...
        {isLoading ? (
          ...
          <TableRow>
            <TableCell colSpan={10} className="py-10 text-center text-sm text-slate-500">
              No products matched the selected city and filters.
            </TableCell>
          </TableRow>
        ) : (
          rows.map((row) => {
